...
class SubscriptionBuilder
{
     ...
     * @throws \Laravel\Cashier\Exceptions\IncompletePayment
     */
    public function create($paymentMethod = null, array $customerOptions = [], array $subscriptionOptions = [], array $stripeOptions = [])
    {
        if (empty($this->items)) {
            throw new Exception('At least one price is required when starting subscriptions.');
        }

        $stripeCustomer = $this->getStripeCustomer($paymentMethod, $customerOptions, $stripeOptions);

        $stripeSubscription = $this->owner->stripe($stripeOptions)->subscriptions->create(array_merge(
     ...
     */
    protected function getStripeCustomer($paymentMethod = null, array $options = [], array $stripeOptions = [])
    {
        $customer = $this->owner->createOrGetStripeCustomer($options, $stripeOptions);

        if ($paymentMethod) {
            $this->owner->updateDefaultPaymentMethod($paymentMethod, $stripeOptions);
        }
