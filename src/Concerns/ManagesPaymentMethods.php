...
trait ManagesPaymentMethods
{
     ...
     */
    public function createSetupIntent(array $options = [], array $stripeOptions = [])
    {
        return $this->stripe($stripeOptions)->setupIntents->create($options);
    }
     ...
     */
    public function addPaymentMethod($paymentMethod, array $stripeOptions = [])
    {
        $this->assertCustomerExists();

        $stripePaymentMethod = $this->resolveStripePaymentMethod($paymentMethod, $stripeOptions);
     ...
     */
    public function defaultPaymentMethod(array $stripeOptions = [])
    {
        if (! $this->hasStripeId()) {
            return;
        }

        $customer = $this->asStripeCustomer(['invoice_settings.default_payment_method'], $stripeOptions);
     ...
     * @return \Laravel\Cashier\PaymentMethod
     */
    public function updateDefaultPaymentMethod($paymentMethod, array $stripeOptions = [])
    {
        $this->assertCustomerExists();

        $customer = $this->asStripeCustomer([], $stripeOptions);
        ...
        $paymentMethod = $this->addPaymentMethod($stripePaymentMethod, $stripeOptions);
     ...
     */
    protected function resolveStripePaymentMethod($paymentMethod, array $stripeOptions = [])
    {
        if ($paymentMethod instanceof StripePaymentMethod) {
            return $paymentMethod;
        }

        return $this->stripe($stripeOptions)->paymentMethods->retrieve($paymentMethod);
    }
}
